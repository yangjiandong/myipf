...
SUJS.View.WebTbTopicsList.prototype.buildTopicsList=function(){var q=this._elements.topicsList;
var n=this._topics.topics;
var g=this._model.getCategory();
var m="<table><tr><td>";
var l="</td></tr></table>";
var e="</td><td>";
var h=9;
if(g.category=="video_channels"){h=8
}function c(A,v){var w=A;
var y=b.length;
var z=Math.ceil(y/h);
var u=Math.ceil(y/z);
var s=u+1;
...
var o=[];
q.html("");
if(g.category=="your_topics"){b=n.ordering.personal
}else{if(g.category=="Popular_Topics"){b=n.ordering.popular
}else{if(g.category=="stumblethru_channel"){b=n.ordering.stumblethru
}else{if(g.category=="video_channels"){b=n.ordering.videochannels
}else{b=n.ordering.alltopics
}}}}for(var f=0,r=b.length;
f<r;
f++){var d={};
if(g.category=="stumblethru_channel"){d=n.stumblethru[b[f]];
d.id=b[f];
d.className="stumbleThru";
o.push(this.buildThruList(d,d.tb_desc))
}else{if(g.category=="video_channels"){d=n.videochannels[b[f]];
d.id=n.videochannels[b[f]].id;
d.className="videoTopic";
d.title=b[f];
o.push(this.buildThruList(d,d.name))
}else{d=n.alltopics[b[f]];
d.id=b[f];
o.push(this.buildList(d))
}}}o=c(o,e);
if(g.category=="your_topics"){var k=[];
k.push('<a class="channel" id="channelSearchBox" href="#"><input type="text" name="tag" id="tagSearchbox"/></a>');
k.push('<a class="channel" id="channelAll" href="#">All</a>');
k.push('<a class="channel" id="channelFollowing" href="#">Following</a>');
k.push('<a class="channel" id="channelPhotos" href="#">Photos</a>');
k.push('<a class="channel" id="channelVideo" href="#">Video</a>');
k.push('<a class="channel" id="channelNews" href="#">News</a>');
k.push("</td><td>");
o.unshift(k.join(""));
if(personalized){o.push('<a class="edit" id="editTopics" href="/settings/interests/" target="_top">Edit Topics</a>')
}}else{if(g.category=="video_channels"){b=n.ordering.videoproviders;
var k=[];
var a=1;
k.push('<a href="#" class="channel" id="channelVideo">All Videos</a>');
if(personalized){k.push('<a href="#" class="channel" id="channelVideoFriendsFavorites">Friends\' Favorites</a>')
}for(var f=0,r=b.length;
f<r;
f++){var d={};
d=n.videoproviders[b[f]];
d.id=b[f];
d.className="videoChannel "+n.videoproviders[b[f]].sname;
k.push(this.buildThruList(d,d.tb_desc));
if(f>0&&(f+3)%h==0){k.push(e);
a++
}}k.push(e);
if(personalized){o.push('<a class="edit" id="editTopics" href="/settings/interests/?pre=video" target="_top">Edit Topics</a>')
}var p=1;
for(var f=0,r=o.length;
f<r;
f++){if(o[f]==e){p++
}}o.unshift(k.join(""));
var j="Stumble videos by topic...";
if(n.ordering.videochannels.length==0){j=""
}o.splice(0,0,"Stumble through...</td>",'<td class="subtitle" colspan="'+p+'">'+j+"</td></tr><tr><td>");
m='<table><tr><td class="subtitle" colspan="'+a+'">'
}}o.push(l);
o.splice(0,0,m);
q.html(o.join(" "));
if(is_su_team){$("#channelSearchBox").show();
$("#tagSearchbox").keydown(function(s){var i=$("#tagSearchbox").val();
if(s.keyCode=="13"&&i.length>0){if(topicSelector&&topicSelector.handleTagStumblingStart){topicSelector.handleTagStumblingStart(i)
}}})
}};
